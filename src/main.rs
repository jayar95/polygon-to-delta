...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let initial_data_sample = polygon_s3
        .build()
    ...
    let (format, schema) = get_arrow_meta_from_cursor(*initial_data_sample)?;
    ...
    for year in start_year..=end_year {
        for month in start_month..=end_month {
            ...
            let path = Path::from(format!("us_stocks_sip/minute_aggs_v1/{year}/{month_dir}/"));
...
async fn decompress_and_write_from_s3_to_delta(
    ...
    let table = delta_s3
        .write_to_delta(data, delta_uri, schema, format, save_mode)
        .await
...
#[instrument(skip_all)]
fn get_arrow_meta_from_cursor(cursor: Cursor<Vec<u8>>) -> Result<ArrowMeta, anyhow::Error> {
    let format = Format::default().with_header(true);
    let (schema, _) = format.infer_schema(cursor, Some(10))?;
