...
static S3_RETRY_INITIAL_INTERVAL: Duration = Duration::from_secs(
	S3_RETRY_INITIAL_SLEEP_SECONDS
);
static S3_RETRY_MAX_INTERVAL: Duration = Duration::from_secs(
	S3_RETRY_INITIAL_SLEEP_SECONDS * 4
);
...
pub trait DeltaIO: ObjectStore {
	...
	async fn write_to_delta(
		...
		format: Format,
		save_mode: SaveMode,
	) -> DeltaResult<DeltaTable> where Self: Sized {
